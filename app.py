        ...
        if posts.empty:
            ...
            for _, row in posts.iterrows():
                ...
                if row['is_notice']:
                    ...
                    c1.markdown('<div style="background-color:#FFEBEE; color:#D32F2F; padding:2px 5px; border-radius:8px; font-weight:bold; text-align:center; font-size:0.8rem;">공지</div>', unsafe_allow_html=True)
...
# [업데이트] 웹디자이너 제안 스타일 적용: 버튼 색상 #0068c9 및 가로 길이 100% 통일

# [추가 요청 반영] 커뮤니티 게시판 글제목 연한 회색으로 변경 & 반응형 폰트/비율 유지 설정

st.markdown("""

<style>

    /* 전체 브라우저 크기에 따른 반응형 설정 (전체 비율 및 폰트 축소 유지) */
    ...
    /* 엑셀 템플릿/샘플 다운로드 버튼 및 전체 버튼 가로 길이 100% 통일 */
    ...
    /* 실행 및 일반 버튼 스타일링 (요청하신 #0068c9 컬러 적용) */
    ...
    /* 커뮤니티 게시판 글제목 버튼 텍스트 색상 변경 (파란색 -> 연한 회색) */

    div[key^="btn_title_"] button p {

        color: #9E9E9E !important;

    }

    /* 버튼 배경색은 유지하되 제목 글자만 연한 회색으로 적용 */
